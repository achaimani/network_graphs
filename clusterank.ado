*! version 2.0.1, 17.09.2019
// version 2.0.0, 09.01.2018
//version 1.2, 26.02.2015, Anna Chaimani

program clusterank
	syntax varlist (min=2 max=3) [if] [in], [BEST(string) METHod(string) CLusters(string) SCATteroptions(string) DENdrogram Weight(string)]
	...
	if "`clusters'"!=""{
		...
		forvalues i=1/`=`=`_nallobs''-1'{
			...
			if _rc==0{
				if "`weight'"!=""{
					qui keep _gr`i' _clus_1* `t' `y1' `y2' `ww'
				}
				else{
	...
	if "`dendrogram'"!=""{
		...
		if "`weight'"!=""{
			local _scat1 `"sc `y2' `y1' [w=`ww'] if _gr==1 || sc `y2' `y1' if _gr==1"'
			forvalues i=2/`=`nclus''{
				local _scat`i' `" `_scat`=`i'-1'',mlab(`t') `scatteroptions' || sc `y2' `y1' [w=`ww'] if _gr==`i' || sc `y2' `y1' if _gr==`i'"'
			}
		}
		else{
