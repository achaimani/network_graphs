*! version 2.0.2, 04.12.2019
// corrected degrees of freedom
// version 2.0.1, 04.07.2019
// bug corrected resulting when option range() was specified
...
program intervalplot
	...
	if "`nomvmeta'"==""{
		...
		cap local df_het:char _dta[network_df_heterogeneity]
		cap local df_inc:char _dta[network_df_inconsistency]
		tokenize `trts'
		local tr1 `ref'
		forvalues i=1/`dim'{
			local tr`=`i'+1' ``i''
		...
		qui{
			...
			local stu:char _dta[network_studyvar]
			...
			tempvar snc sna tsna
			by `stu',sort:gen `snc'=_N
			gen `sna'=(1+sqrt(1+8*`snc'))/2
			by `stu',sort:replace `sna'=. if _n>1
			egen `tsna'=sum(`sna')
			local tna=`tsna'
			local df=0
			cap local df=`df_het'+`df_inc'
			if `df'<1{
				local df=`tna'-`ns'-(`ntm'-1)
			}             
		}
		if "`reference'"==""{
