*! version 2.0.2, 04.07.2019
//bug corrected resulting in wrong graph with the if option
...
program netfunnel
	...
	lab val `tt2' _trt
	qui gen `estnew'=.
	qui replace `estnew'=`est' if `tt1'==`tnew1' & `use'==1
	qui replace `estnew'=-`est' if `tt1'==`tnew2' & `use'==1
	tempvar c nsc
	qui{
		egen `c'=concat(`ttt1' `ttt2') if `use'==1,punct(_)
		cap drop _dc*
		tab `c',gen(_dc)
		tempname DC
		mkmat _dc*,matrix(`DC')
	...
	cap{
		twoway `_scat`nc'' , yscale(rev) mc(black) xtitle(`xtitle') ytitle(`ytitle') xline(0, lcol(red)) ylab(`ylabel') xlab(`xlabel') `scatteroptions'|| `_lines' || `addplot', caption(Note: Comparisons including only one study (when present) have been excluded, justification(left) size(vsmall) margin(medium))
	}
	if _rc!=0{
	...
	di as res _newline "* Note: Comparisons including only one study (when present) have been excluded from the graph"
	qui sort `rankstart'
	tempname EC
