*! version 2.0.2, 07.12.2019
//bug fixed when treatment codes used
// version 2.0.2, 06.08.2019
...
program netleague
	...
	if "`nomvmeta'"==""{
		...
		cap lab drop _trt
		cap lab drop _ttrt	
		cap tostring `tt1' `tt2',replace format("%02.0f")
	...
	forvalues j=1/`ntm'{
		cap rename _`tr`j''_new3 _t_`tr`j''
	}
	forvalues j=1/`ntm'{
		forvalues i=1/`ntm'{
			if `ord`i''==`j'{	
				cap replace _t_`tr`i''="`lab`i''" in `j'
			}
		}
	}
	if "`nomvmeta'"==""{
		cap drop _Comparison 
		cap drop `y' `lci' `uci' `tt1' `tt2'
		cap drop _LPrI _UPrI
	}	
	forvalues i=1/`ntm'{
		if `ord`i''==1{			
			local keep1 `" "_t_`tr`i''" "'
		}
	}
	forvalues i=1/`ntm'{
		if `ord`i''==`ntm'{			
			local keep2 `" "_t_`tr`i''" "'
		}
	}
	tempname kp1 kp2
	scalar `kp1'=`keep1'
