...
// bug in node weights corrected resulting when changing reference 
	...
	qui{
		...
		if "`nodeweight'"!=""{
			...
			if "`_nwfun'"=="" | "`_nwfun'"=="sum"{
				forvalues i=1/`ntm'{
					tempvar _treat`i' _str`i'
					gen `_treat`i''=`_nwvar1' if `told1'==`i'
					if "`_nwvar2'"!=""{
						replace `_treat`i''=`_nwvar2' if  `told2'==`i'
					}
					else{
						replace `_treat`i''=`_nwvar1' if  `told2'==`i'
					}
					replace `_treat`i''=0 if `_treat`i''==. & (`told1'==`i' | `told2'==`i')
					egen `_str`i''=sum(`_treat`i'')
				}
			}
			else if "`_nwfun'"=="mean"{
