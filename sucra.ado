*! version 2.0.1, 16.09.2019
// the POST-R measure for treatment ranking has been incorporated
...
program sucra
	...
	LEAGUEtable EXPort(string asis) SORT(string asis) POSTR(string)]
		
	preserve
	
	if "`postr'"!=""{
		local pscore "pscore"
		local leaguetable "leaguetable"
	}	
	if "`pscore'"!="" & "`nomvmeta'"!=""{
	...
	if "`postr'"!="" & `"`sort'"'==""{
		di as err "Option {it:postr()} requires option {it:sort()}"
		exit
	}
	if "`postr'"!="" & "`min'"=="" & "`max'"==""{
		di as err "Option {it:postr()} requires option {it:min/max}"
		exit
	}
	...
	/*calculate the cumulative ranking probabilities and the SUCRA values*/
		...
		if "`noplot'"=="" {
			...
			/*produce SUCRA plots*/
				...
				mat _CP2=vec(_CP)
				cap mat _CPP2=vec(_CPP)
				mat _D2=vec(_D)
				mat _R2=vec(_R)
				local ob=`ntmc'^2
			}
			cap set obs `ob'
			qui svmat _D2,names(_tr)
			qui svmat _R2,names(_rt)
			cap mat drop _D2
			cap mat drop _R2
			cap mat drop _D
			cap mat drop _R
			...
			cap svmat _P2,names(_pr)
			cap svmat _CP2,names(_cpr)
			cap mat drop _P2
			cap mat drop _CP2
			cap mat drop _P
			cap mat drop _CP
			...
			if "`_s2'" != "" | "`compare'" !=""{
				cap svmat _PP2,names(_pr2)
				cap svmat _CPP2,names(_cpr2)
				cap mat drop _PP2
				cap mat drop _CPP2
				cap mat drop _PP
				cap mat drop _CPP				
	...
	if "`pscore'"!=""{
		...
		if "`leaguetable'"!=""{
			...
			if "`postr'"!=""{
				qui destring P_score,replace
				forvalues i=1/`ntm'{
					qui replace P_score in `i'=`psc`i''
				}
				tempvar lastsort
				qui gen `lastsort'=.
				forvalues i=1/`ntm'{
					qui replace `lastsort'=`i' if Treatment=="`tsort`i''"
				}
				qui sort `lastsort'
				qui mkmat P_score if P_score!=.,mat(psc)
			}
			restore
			cap drop _treat*
			forvalues i=1/`ntm'{
			...
			if "`postr'"!=""{
				qui gen Treatment=""
				forvalues i=1/`ntm'{
					qui replace Treatment in `i'="`tsort`i''"
				}
			}
		}
	}
	...
	if "`postr'"!=""{
		tokenize `postr'
		if "`2'"==""{
			di as err "Option {it:postr()} requires two arguments: the initial probability distribution vector and the probability for continuing the selection process"
			exit
		}
		local q `1'
		local z `2'
		...
		di as text _newline " {bf:Note}: The sucra command assumes that the probabilities in `q' follow the order of the treatments as specified in option {it:sort()}"
		forvalues i=1/`ntm'{
			qui replace _treat`i'="" in `i'
			qui destring _treat`i',replace
			qui replace _treat`i'=0 in `i'
		}
		tempvar sum
		qui egen `sum'=rowtotal(_treat*) if _treat1!=.
		forvalues i=1/`ntm'{
			qui replace _treat`i'=_treat`i'/`sum'
		}
		mkmat _treat* if _treat1!=.,mat(_A)
		mat _Q=`q'
		scalar _ntm=`ntm'
		scalar _z=`z'
		...
		qui svmat psc
		qui rename psc P_score
		qui gen postr=string(round(POST_R*100,0.1),"%04.1f") if POST_R!=.
		qui gen pscore=string(round(P_score*100,0.1),"%04.1f") if P_score!=.
		qui drop POST_R P_score
		qui rename postr POST_R
		qui rename pscore P_score

		if "`order'"==""{
			qui sort `ord'
			li Treatment P_score POST_R if P_score!="",table div noobs separator(0)	
		}
		if "`order'"!=""{
			qui gsort -P_score
			li Treatment P_score POST_R if P_score!="",table div noobs separator(0)
		...
		if `"`export'"'!=""{
			qui export excel POST_R if POST_R!="" using `export' , sheet("POST-R") firstrow(var) sheetmodify 
		}
		drop POST_R P_score Treatment
		mat drop psc
	}
